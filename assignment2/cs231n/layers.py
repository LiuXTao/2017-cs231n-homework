...
def conv_forward_naive(x, w, b, conv_param):
  ...
  x_padded=np.pad(x,((0,),(0,),(padding,),(padding,)),'constant') # pad alongside four dimensions
  N,C,H,W=x.shape
  F,C,HH,WW=w.shape
  ...
  '''
  第一个维度是 batch，第二个维度 k 是输出通道数，后两个维度 i，j 是 feature map 的高和宽
  '''
...
def conv_backward_naive(dout, cache):
  ...
  db=np.sum(dout,axis=(0,2,3))    #b只对第二维有意义
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
  ...
  out=out.reshape(N,H,W,C).transpose(0,3,1,2)  # 维度转变回来
