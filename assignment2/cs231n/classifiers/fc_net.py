...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    hidden_layers, cache=list(range(self.num_layers+1)),list(range(self.num_layers))
    ...
    for i in range(self.num_layers):
      ...
      W,b =self.params['W'+str(i+1)],self.params['b'+str(i+1)]
      if  i ==self.num_layers-1:
        hidden_layers[i+1],cache[i]=affine_forward(hidden_layers[i],W,b)
      else:
        if self.use_batchnorm:
          gamma, beta = self.params['gamma' + str(i + 1)], self.params['beta' + str(i + 1)]
          hidden_layers[i + 1], cache[i] = affine_bn_relu_forward(hidden_layers[i], W, b, gamma, beta,
                                                                   self.bn_params[i])
        else:
          hidden_layers[i+1],cache[i]=affine_relu_forward(hidden_layers[i],W,b)

        if self.use_dropout:
          hidden_layers[i+1],dp_cache[i]=dropout_forward(hidden_layers[i+1],self.dropout_param)
    scores=hidden_layers[self.num_layers]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    for i in range(self.num_layers,0,-1):
      if i== self.num_layers:
        ...
        if self.use_batchnorm:
          ...
          dx,dw,db=affine_relu_backward(dhidden[i],cache[i-1])
          dhidden[i-1],grads['W'+str(i)],grads['b'+str(i)]=dx,dw,db

      loss+=0.5*self.reg*np.sum(self.params['W'+str(i)]**2)  # loss正则化
