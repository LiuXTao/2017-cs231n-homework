...
class CNN(object):
    ...
    '''
    conv-relu-conv-relu-pool]x2 - [affine-relu]x2 - [softmax or SVM


    '''

    def __init__(self,input_dim=(3,32,32),kernel_one=(32,5,5),
                 hidden_dim=100,num_classes=10,weight_scale=1e-3,dropout=0,
                 use_batchnorm=False,seed=None,reg=0.01,dtype=np.float32):
        ...
        self.params['W5']=weight_scale*np.random.randn(32*8*8,hidden_dim)
    ...
    def loss(self,X,y=None):
        ...
        # print(0,X.shape)
        # print(W1.shape)
        conv1_out,conv1_cache=conv_relu_forward(X,W1,b1,conv_param1)
        # print(1,conv1_out.shape)
        # print(W2.shape)
        conv2_out,conv2_cache=conv_relu_pool_forward(conv1_out,W2,b2,conv_param1,pool_param1)

        # print(2,conv2_out.shape)
        # print(W3.shape)
        conv3_out,conv3_cache=conv_relu_forward(conv2_out,W3,b3,conv_param2)
        # print(3,conv3_out.shape)
        # print(W4.shape)
        conv4_out,conv4_cache=conv_relu_pool_forward(conv3_out,W4,b4,conv_param2,pool_param2)
        # print(conv4_out.shape)
        # print(W5.shape)
        fc1_out,fc_cache=affine_relu_forward(conv4_out,W5,b5)
        # affine_out,drop_cache=dropout_forwad(affine_out,self.dropout_param)
        scores,fc2_cache=affine_forward(fc1_out,W6,b6)
        ...
        dfc1,grads['W6'],grads['b6']=affine_backward(dscore,fc2_cache)

        # dfc1=dropout_backward(dfc1,drop_cache)
        dconv4,grads['W5'],grads['b5']=affine_relu_backward(dfc1,fc_cache)
        dconv3,grads['W4'],grads['b4']=conv_relu_pool_backward(dconv4,conv4_cache)
        dconv2,grads['W3'],grads['b3']=conv_relu_backward(dconv3,conv3_cache)
        dconv1,grads['W2'],grads['b2']=conv_relu_pool_backward(dconv2,conv2_cache)
        dx,grads['W1'],grads['b1']=conv_relu_backward(dconv1,conv1_cache)

        # for i in range(6):
        #     print(grads['W'+str(i+1)].shape)

        # 正则化
        for i in range(6):

            loss+=0.5*self.reg*np.sum(self.params['W'+str(i+1)]**2)
            # print(grads['W'+str(i+1)].shape,self.params['W'+str(i+1)].shape)
